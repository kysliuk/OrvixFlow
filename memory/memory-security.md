...
## 10. Remediation Status Reference

For the complete findings, fixes, and rationale, see `tasks/claude-security-review-08-04.md`.

### Summary of What Was Fixed and When

| Phase | Date | Findings Fixed |
|-------|------|---------------|
| Phase 1 | 2026-04-09 | F-09 (X-Tenant-ID), F-10 (impersonation logging), F-15 (secrets to .env), F-17 (JWT secret from appsettings), F-31 (Groq key), F-02 (OAuth linking) |
| Phase 2 | 2026-04-09 | F-32 (security headers), F-11 (magic byte MIME), F-12 (filename sanitization), F-14 (storage error logging), F-22 (Hangfire auth), F-01 (JWT 60min), F-03 (login rate limit), F-28 (security log levels), F-16 (FixedTimeEquals), F-08 (invite role ceiling), F-19 (WebhookSecret removed) |
| Phase 3 | 2026-04-14 | F-06 (async RequireModule), F-25 (admin server-side guard), F-07 (user-level module check), F-27 (AI endpoint rate limits), F-04 (password complexity), F-33 (email verification), F-21 (audit retention), F-23 (n8n network segmentation) |
| Phase 4 | 2026-04-14 | F-05 (invite token via email only), F-13 (text ingestion size limit), F-18 (Noop virus scan startup warning), F-24 (per-tenant workflow rate limit), F-29 (server-side API proxy routes), F-26 (audit trail for destructive actions — already present) |

### Still Open

| ID | Description | Priority |
|----|-------------|----------|
| F-20 | MinIO/S3 storage (replaces volume mount) | Deferred — separate PR |
| F-29 | Full migration of all API calls to server-side Route Handlers | Low |
| F-30 | Stale role in session after admin role change | Low |
