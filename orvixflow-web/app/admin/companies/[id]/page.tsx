...
export default function CompanyDetailPage() {
  ...
  const handleRestoreArchivedCompany = async () => {
    if (!confirm("Restore this archived company and make it accessible again?")) return;

    setActionLoading("restore");
    setActionError(null);
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/companies/${companyId}/restore`, {
      method: "POST",
      headers: getHeaders()
    });
    setActionLoading(null);
    ...
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
      setActionError(data.error || "Failed to restore archived company");
      return;
    }
    ...
    <div className="flex flex-col gap-6 max-w-6xl h-full">
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <span className={`px-3 py-1 rounded-full text-xs font-bold ${company.company.lifecycleStatus === "Archived" ? "bg-warning/10 text-warning" : "bg-success/10 text-success"}`}>
            {company.company.lifecycleStatus}
          </span>
        </div>
      </div>

      {company.company.lifecycleStatus === "Archived" && (
        <div className="bg-warning/10 border border-warning/30 p-4 rounded-xl flex items-start gap-3">
          <AlertTriangle className="w-5 h-5 text-warning mt-0.5" />
          <div>
            <h3 className="font-medium text-warning">Company Archived</h3>
            <p className="text-sm text-white/70">
              Archived {company.company.archivedAt ? formatDate(company.company.archivedAt) : "recently"}. Scheduled for permanent purge on {company.company.deletionScheduledFor ? formatDate(company.company.deletionScheduledFor) : "unknown"} unless restored first.
            </p>
          </div>
        </div>
      )}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-black/40 border border-danger/20 rounded-xl p-5">
              ...
              <div className="space-y-2">
                ...
                {company.company.lifecycleStatus === "Archived" && (
                  <button
                    onClick={handleRestoreArchivedCompany}
                    disabled={actionLoading === "restore"}
                    className="w-full px-4 py-2 bg-success/10 text-success text-sm font-medium rounded-lg hover:bg-success/20 transition-colors disabled:opacity-50"
                  >
                    {actionLoading === "restore" ? "Restoring..." : "Restore Archived Company"}
                  </button>
                )}
              </div>
            </div>
